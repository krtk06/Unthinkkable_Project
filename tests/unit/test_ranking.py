...
def match(candidate_id: str, score: float, skills: float = 1.0) -> MatchResult:
    return MatchResult(
        ...
        analysis="",
        shortlisted=score >= 7.0,
        model=ModelMetadata(provider="test", model="test", prompt_version="v1"),
    )


def test_ranks_by_score_then_skills_then_candidate_id() -> None:
...
def test_filters_score_skills_and_location() -> None:
    ...
    filtered = rank_matches(
        results,
        filters={"min_score": 8, "min_skills_score": 0.9, "location": "London"},
        metadata=metadata,
    )
